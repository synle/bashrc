 ...
 *
 * @param {{sha: string, fileCount: number, payloadBytes: number}} info - build metadata for the banner.
 * @returns {string} bash header text, including the trailing payload marker line.
 */
function buildHeader({ sha, fileCount, payloadBytes }) {
...
# so the awk-based payload extraction below can read \$0 as a file.
...
function main() {
  ...
  console.log(
    `>> built ${path.relative(REPO_ROOT, OUTPUT_PATH)} (${(finalSize / 1024).toFixed(1)} KB, payload ${(tarball.length / 1024).toFixed(1)} KB, ${fileCount} files)`,
  );

  return { outputPath: OUTPUT_PATH, payloadBytes: tarball.length, fileCount };
}
