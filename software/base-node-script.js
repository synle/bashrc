...
function writeText(filePath, text, override = true, suppressError = false) {
  ...
  const oldContentStripped = (commentBreakIdx_old >= 0 ? oldContent.substring(oldContent.indexOf('\n', commentBreakIdx_old) + 1) : oldContent).trim();
  const newContentStripped = (commentBreakIdx_new >= 0 ? newContent.substring(newContent.indexOf('\n', commentBreakIdx_new) + 1) : newContent).trim();
...
function getAutoGeneratedText(){
  ...
  const timehere = `${yyyy}-${mm}-${dd} ${hh}${ampm}`;
