...
async function doWork() {
  const targetPath = "software/metadata/autocomplete-complete-spec/npm";
  ...
  try {
    ...
    // each command becomes a line, plus static overrides with dynamic tokens
    const res = commands.map((cmd) => `${cmd}|`);
    res.push("run|__npm_scripts__");
    const newContent = dedupeSpecLines(res);
