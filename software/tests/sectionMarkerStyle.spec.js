...
/** File extensions the JS-family rule covers (JS / TS / JSONC). */
const SCAN_EXTS = new Set([".js", ".jsx", ".ts", ".tsx", ".jsonc"]);

/** File extensions the shell rule covers. */
const SHELL_EXTS = new Set([".sh", ".bash"]);

/**
 * Walks the repo and returns every file whose extension is in `exts`,
 * skipping SKIP_DIRS.
 * @param {string} dir - Directory to recurse into
 * @param {Set<string>} exts - Extensions to collect
 * @param {string[]} out - Accumulator (mutated)
 * @returns {string[]} List of absolute file paths
 */
function walk(dir, exts = SCAN_EXTS, out = []) {
...
describe("section marker style", () => {
  ...
  it("rejects legacy `# ---- Title ----` section markers in shell files", () => {
    const files = walk(ROOT_DIR, SHELL_EXTS);
    /** @type {string[]} */
    const offenders = [];
    for (const file of files) {
      const lines = fs.readFileSync(file, "utf8").split("\n");
      for (let i = 0; i < lines.length; i++) {
        // Canonical style is exactly three dashes on each side: `# --- Title ---`.
        // Anchored at column 0 so that markers echoed inside strings (which are
        // always indented) are not flagged.
        if (/^# -{4,} .* -{4,}$/.test(lines[i])) {
          offenders.push(`${path.relative(ROOT_DIR, file)}:${i + 1}`);
        }
      }
    ...
    expect(
      offenders,
      `Found legacy '# ---- Title ----' section markers. Convert to '# --- Title ---' per AGENTS.md.\n` +
        offenders.map((o) => `  ${o}`).join("\n"),
    ).toEqual([]);
  });
});
