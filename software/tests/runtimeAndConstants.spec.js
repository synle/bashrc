...
describe("constants", () => {
  ...
  it("should have TEMP_SCRIPT_PREFIX derived from BASHRC_TEMP_ROOT_DIR when BASHRC_TEMP_DIR is unset", () => {
    expect(TEMP_SCRIPT_PREFIX).toBe("/tmp/test/bashrc/sw_");
  });

  it("should have BASHRC_TEMP_ROOT_DIR from sandbox env", () => {
    const BASHRC_TEMP_ROOT_DIR = getIndexConstant("BASHRC_TEMP_ROOT_DIR");
    expect(BASHRC_TEMP_ROOT_DIR).toBe("/tmp/test/bashrc");
  });

  it("should have _URL_CACHE_DIR derived from BASHRC_TEMP_ROOT_DIR", () => {
    const _URL_CACHE_DIR = getIndexConstant("_URL_CACHE_DIR");
    expect(_URL_CACHE_DIR).toBe("/tmp/test/bashrc/url_cache");
  });
