...
describe("_getAutoColor", () => {
  ...
  // ---- ## marker (bold foreground colors) ----
  it("should return colorBoldYellow for ## (level 1)", () => {
    expect(getColorCode("## header")).toBe(CODES.boldYellow);
  });

  it("should return colorBoldCyan for ### (level 2)", () => {
    expect(getColorCode("### header")).toBe(CODES.boldCyan);
  });

  it("should return colorBoldMagenta for #### (level 3)", () => {
    expect(getColorCode("#### header")).toBe(CODES.boldMagenta);
  });

  it("should return colorOrange for ##### (level 4+)", () => {
    expect(getColorCode("##### header")).toBe(CODES.orange);
  });
