...
describe("Zed keybindings — no duplicate keys", () => {
  ...
  it("F5 is bound to ReloadActiveItem (revert + reload from disk)", () => {
    const noContext = keybindings.find((e) => !e.context);
    expect(noContext).toBeDefined();
    expect(noContext.bindings["f5"]).toBe("workspace::ReloadActiveItem");
  });

  it("OS_KEY-shift-r is bound to ReloadActiveItem (revert + reload from disk)", () => {
    const noContext = keybindings.find((e) => !e.context);
    expect(noContext).toBeDefined();
    expect(noContext.bindings["OS_KEY-shift-r"]).toBe("workspace::ReloadActiveItem");
  });
});
