...
// Plain readdir rather than fs.globSync — globSync landed in Node 22 and this
// repo still supports Node 20, where it is undefined and the suite fails to load.
const ADVANCED_DIR = path.join(ROOT_DIR, "software/scripts/advanced");
const commonFiles = fs
  .readdirSync(ADVANCED_DIR)
  .filter((f) => f.endsWith(".common.js"))
  .sort()
  .map((f) => path.join(ADVANCED_DIR, f));
