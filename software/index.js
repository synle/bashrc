...
function parseRawArgs() {
  ...
  if (presetRequests.length > 0) {
    ...
    for (const { name, viaAtMarker } of presetRequests) {
      ...
      const resolvedName = _resolvePresetName(name, presetMap, {
        suggestionFlag,
      });
      if (!resolvedName) {
...
/**
 * Builds the shared `[date]` / `[date] [n chars]` stamp used by both the opening and
 * closing auto-generated markers, so the two can never disagree on format.
 * @param {number} [charCount] - Character count of the content the marker describes. Omitted when not a finite number.
 * @returns {string} e.g. `[2026-08-13]` or `[2026-08-13] [55780 chars]`
 */
function _getAutoGeneratedStamp(charCount) {
  const now = new Date();
  const yyyy = now.getFullYear();
  const MM = String(now.getMonth() + 1).padStart(2, "0");
  const dd = String(now.getDate()).padStart(2, "0");

  /** @type {string} " [12345 chars]" when a count was supplied, otherwise empty. */
  const charSuffix = Number.isFinite(charCount) ? ` [${charCount} chars]` : "";

  return `[${yyyy}-${MM}-${dd}]${charSuffix}`;
}
 ...
 *
 * Pass `charCount` for content under a host-imposed size cap (Claude Code refuses
 * a `CLAUDE.md` over 40k chars) so the deployed file states its own size instead
 * of requiring a `wc -c` to find out.
 *
 * @param {number} [charCount] - Character count of the content this header describes. Omitted from the output when not a finite number.
 * @returns {string} A warning string indicating the file is auto-generated, with date and optional char count
 */
function getAutoGeneratedText(charCount) {
  return `AUTO-GENERATED - DO NOT EDIT ${_getAutoGeneratedStamp(charCount)}\n`;
}

/**
 * Closing counterpart to {@link getAutoGeneratedText}. Placed at the bottom of a generated
 * block so a reader scrolling up from the end of a long file sees where the managed region
 * stops, without having to find the machine-facing `END <key>` marker.
 *
 * Pass the same `charCount` given to the opening marker — a mismatch between the two stamps
 * in a deployed file means the block was hand-edited between them.
 *
 * @param {number} [charCount] - Character count of the content this footer closes. Omitted from the output when not a finite number.
 * @returns {string} A closing string for an auto-generated section, with date and optional char count
 */
function getAutoGeneratedEndText(charCount) {
  return `END AUTO-GENERATED ${_getAutoGeneratedStamp(charCount)}\n`;
}
...
async function installLinuxUniversalAppImage(appImagePath, appLabel, sourceFolder) {
  ...
  log(`>> Installing Linux AppImage ${appLabel} from`, appImagePath);
  ...
  const desktopPath = path.join(desktopDir, `${appLabel}.desktop`);
  await mkdir(desktopDir);
  await writeText(desktopPath, desktopContent);
...
function registerWithPowershellProfile(configKey, content) {
  registerProfileBlock({
    profilePath: POWERSHELL_SYLE_PATH,
    configKey,
    content,
  });
}
...
function _resolveScriptFile(file, originalFile, allRepoFiles) {
  ...
  return {
    resolvedFile: fileExists ? foundMatchedPath : file,
    fileExists,
    description,
    fileMatchState,
  };
...
function _resolveBareArgPresetFallback(softwareFiles, allRepoFiles) {
  ...
  for (const entry of softwareFiles) {
    ...
    const resolvedName = _resolvePresetName(entry, presetMap, {
      suggestionFlag: "@",
    });
    if (resolvedName) {
