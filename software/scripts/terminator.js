...
async function doWork() {
  ...
  const content = code`
    ...
    [keybindings]
      ...
      copy = <Alt>c
      paste = <Alt>v
      toggle_scrollbar = <Alt>backslash
      search = <Alt>f
      close_window = <Alt>q
      resize_up = <Shift><Alt>Up
      resize_down = <Shift><Alt>Down
      resize_left = <Shift><Alt>Left
      resize_right = <Shift><Alt>Right
      move_tab_right = ""
      move_tab_left = ""
      toggle_zoom = F11
      scaled_zoom = ""
      next_tab = <Alt>braceright
      prev_tab = <Alt>braceleft
  ...
  await writeBuildArtifact([
    {
      file: `${BUILD_DIR}/terminator-config`,
      data: `
...
${content}
  ...
  if (is_os_windows || is_os_mac || is_os_arch_linux) {
    log(">>> Skipped");
    return;
  }
