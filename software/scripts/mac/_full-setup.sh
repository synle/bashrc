...
# _appArchMismatch <App Name.app> - Returns 0 when an installed app bundle has no slice
# for the machine's native CPU arch, i.e. an Intel-only app that would run translated
# under Rosetta 2 on Apple Silicon. Resolves the bundle's main executable through
# CFBundleExecutable, falling back to the first executable in Contents/MacOS.
# Returns 1 (no mismatch) when the app or its executable cannot be resolved, so an
# unreadable bundle never triggers a pointless reinstall.
function _appArchMismatch() {
  local app="/Applications/$1"
  [ -d "$app" ] || return 1
  local exe _candidate
  exe=$(defaults read "$app/Contents/Info" CFBundleExecutable 2> /dev/null)
  if [ -z "$exe" ] || [ ! -x "$app/Contents/MacOS/$exe" ]; then
    exe=""
    for _candidate in "$app/Contents/MacOS/"*; do
      [ -x "$_candidate" ] && [ -f "$_candidate" ] || continue
      exe="${_candidate##*/}"
      break
    done
  fi
...
function installBrewPackage() {
  ...
  # fast path: check /Applications directly, no brew call needed. An app that is
  # installed but has no slice for the native CPU arch is reinstalled instead of
  # skipped — brew resolves x86_64 artifacts whenever it runs translated under
  # Rosetta 2, which leaves Intel-only apps behind on Apple Silicon.
  local _arch_reinstall=0
  if [ -n "$app_name" ] && [ -d "/Applications/$app_name" ]; then
    if _appArchMismatch "$app_name"; then
      _arch_reinstall=1
      echo -n "Reinstalling (not built for $(get_native_arch)) >> "
    else
      echo "Skipped"
      return
    fi
...
# a `_<bucket>` suffix so `tail -f $BASHRC_TEMP_ROOT_DIR/bashrc_bg_brew_$$_*.log` watches them all.
_BACKGROUND_NUM_BUCKETS=4
_BACKGROUND_INSTALL_LOG_PREFIX="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_brew_$$"
_BACKGROUND_INSTALL_SCRIPT_PREFIX="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_brew_$$_script"
...
installBrewPackage shellcheck # used by `make ci_test_shellcheck`
