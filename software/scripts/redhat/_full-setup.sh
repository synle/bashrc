...
_BACKGROUND_INSTALL_LOG="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_dnf_$$.log"
_BACKGROUND_INSTALL_SCRIPT="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_dnf_$$.sh"
...
installDnfPackage ShellCheck # used by `make ci_test_shellcheck` (dnf pkg name is capitalized)
...
if ((is_gui)); then
  ...
  if ((is_gui_x11)); then installDnfPackageInBackground xclip; fi
  if ((is_gui_wayland)); then installDnfPackageInBackground wl-clipboard; fi
