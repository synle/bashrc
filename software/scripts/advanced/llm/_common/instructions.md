...
- Automerge is opt-in — never pass `--auto` on your own initiative. Offer once, only when the diff is clearly trivial / tests-only / dependency-only / docs-only, and enable only on an explicit "yes". Outside those four: don't ask, don't enable. Never on a WIP or `DO NOT MERGE` PR. If the user pre-enabled it, leave it alone.
