...
- **PR body file is mandatory.** Create an explicit temporary Markdown file (for example, `/tmp/pr-body.md`) with the file-editing tool before running `gh pr create` or `gh pr edit`. Keep the GitHub CLI command body-free: `gh pr create --base <default> --title "<title>" --body-file /tmp/pr-body.md`. Run the command with the file path, not with generated body contents; remove the file with the file-editing tool only after a successful command, and keep it on failure for diagnosis.
...
- One rigid worktree path, and exactly one thing that computes it — `worktree_create --path-only <branch>` prints `$HOME/.worktrees/<owner>/<repo>/<sanitized-branch>` and touches nothing. `<owner>` and `<repo>` come from the origin remote, never from the folder name. The leaf preserves letters and digits and converts every other character to `_`, collapsing repeats and trimming edges — `syle/oauth-migration/fix~it` becomes `syle_oauth_migration_fix_it`. No PR number, no per-command suffixes, no `mktemp -d`, no sibling-of-repo folders. Clean up with `git clean-worktree` (prunes stale records, removes merged/gone branches, skips dirty ones); it skips detached-HEAD worktrees, so remove those with `git worktree remove`.
