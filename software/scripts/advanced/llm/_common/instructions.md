...
- PR body follows the repo's template when one exists. `gh pr create --body` / `--body-file` **overrides** the template GitHub would pre-fill, so writing a body without looking discards it. Check first:
  ...
  When found, the body **is** that template filled in: every heading reproduced in original order, wording, and level; nothing dropped, nothing invented. Answer `<!-- ... -->` placeholders and delete the comment. A section that doesn't apply gets `N/A` plus a half-line why — on a WIP, `TODO — <what's left>` — never deleted. **Always write the complete body to a file with the file-editing tool, review that file, and pass it with `--body-file <tmp>`.** Never pass body text through `--body`, `--body=`, a shell variable, `echo`, `printf`, command substitution, or an inline shell heredoc; Markdown backticks, quotes, dollar signs, and other syntax must stay out of the shell command.

- **Every GitHub body payload uses a temporary file.** Create an explicit temporary Markdown file with the file-editing tool before `gh pr create`, `gh pr edit`, `gh pr comment`, or `gh pr review`; pass it with `--body-file`. For REST comments, replies, reactions with request data, or review payloads, write a JSON request file and pass it with `gh api --input <file>`. Never pass body text through `--body`, `--body=`, `-f body=`, a shell variable, `echo`, `printf`, command substitution, or an inline shell heredoc. Capture the request result, then delete each temporary file immediately whether the request succeeded or failed; journals are persistent and are not temporary payload files.
- **Every PR workflow keeps a durable journal.** After resolving the PR head branch, call `worktree_create --path-only <head-branch>` and use its exact canonical output to place `pr<number>-<sanitized-branch>.md` beside the worktree under `$HOME/.worktrees/<owner>/<repo>/`. Create or append with the file-editing tool. Record the request, pass number, actions, GitHub result, and next state after each pass; keep the journal across runs. Never hand-build the sanitizer, put transient request bodies in the journal, or delete the journal during cleanup.
...
- **Hard dependencies phase into waves, never into a stack.** Some work has a real ordering constraint no amount of slicing removes — a SQL migration the next PR's code reads, an API field a client consumes, a config key a service needs at boot. Waves are the answer, and the distinction that makes them safe is that **a wave is a merge barrier, not a branch parent**: every PR in every wave is cut from `<default>` and targets `<default>`, and wave N+1's branch simply does not contain wave N's code yet. Nothing is ever based on an open PR, so there is no retarget, no rebase-onto-parent, and no child that GitHub considers unmergeable.
  - **Wave 1 is everything that depends on nothing** — the migration, the schema, the shared interface, the config key. Within a wave, PRs stay disjoint per the rule above and merge in any order, in parallel. Waves are ordered; the PRs inside one are not.
  - **Later waves open as WIP** — `WIP: DO NOT MERGE — [<repo>] ...`, with a `## Blocked on` section in the body naming the wave and linking every PR it waits for. That reference is what already marks the PR non-standalone, so the existing automerge deferral applies with no new mechanism. Reuse one `<group-slug>` across every wave of the feature so the whole set clusters in one place.
  - **A later-wave PR is expected to be red, and that is not a broken build.** Its branch legitimately lacks wave 1's migration, so CI legitimately fails. Say so in the body — `"CI stays red until <link> lands"` — or the redness reads as neglect, and never let a fix loop spend attempts on a failure whose cause is an unmerged dependency.
  - **Promotion order: blocker merged → `git merge origin/<default>` → CI green → drop the WIP prefix.** In that order, and only when the blocked-on section was the PR's _only_ stated reason for being WIP and no `TODO —` markers remain. A WIP whose stated reason has become false is exactly the drift the "PR body tracks the branch" rule exists to catch.
  - **Migrations phase expand → migrate → contract, and wave 1 must be backward compatible.** Wave 1 only adds — a nullable column, a new table, a new index — and leaves currently-deployed code working untouched; that is precisely what lets it land and sit alone in production if wave 2 slips. Wave 2 backfills and dual-writes. Wave 3 drops the old thing, and only once wave 2 is fully deployed. **A rename is never one PR**: add-new → dual-write → backfill → read-new → drop-old. Every wave still ships its own down migration, and destructive waves still carry the `## Recovery` section.
  - **If wave 1 cannot be made backward compatible, the waves are wrong.** Either the work is one PR behind a single atomic deploy, or the schema change needs redesigning. A wave 1 that breaks production the moment it lands is not a phase — it is half a change, and phasing it has bought nothing while adding a window where the system is broken.
...
- Never do PR-branch work in the primary checkout — always a git worktree, so the user's repo and branch are never moved under them. **`worktree_create <branch>` is the only sanctioned creator.** It prints the folder on stdout and diagnostics on stderr, so `WT="$(worktree_create "$BR")"` is the entire incantation. It reuses a linked worktree already on the branch, fetches before creation, preserves unpushed local commits, refuses to return the primary checkout, and falls back to a **detached** worktree when needed. Don't hand-roll `git worktree add`, don't hand-roll the path, and don't `mkdir` the worktree yourself. Remove only worktrees you created. When cwd is not the target repo, clone it outside the worktree container first and run `worktree_create` inside that clone. Interactively, `git_create_worktree` is the same thing plus a `cd`.
- One rigid worktree path, and exactly one thing that computes it — `worktree_create --path-only <branch>` prints `$HOME/.worktrees/<owner>/<repo>/<sanitized-branch>` and touches nothing. `<owner>` and `<repo>` come from the origin remote, never from the folder name. The leaf preserves letters, digits, `.`, `_`, and `-`; every other run becomes `_`, and leading/trailing `.` / `_` / `-` are trimmed — `syle/oauth-migration/fix~it` becomes `syle_oauth-migration_fix_it`. No repo-name prefix, no PR number, no per-command suffixes, no `mktemp -d`, no sibling-of-repo folders. An already-linked worktree on the branch may have an older noncanonical leaf; reuse it without moving or deleting it, report the legacy path, and use `--path-only` as the canonical path for new work and journals. Clean up with `git clean-worktree` (prunes stale records, removes merged/gone branches, skips dirty ones); it skips detached-HEAD worktrees, so remove those with `git worktree remove`.
...
- **A link is four independent facts — owner, repo, `pull` vs `issues`, number — and each one is verified separately.** Well-formedness is not correctness: swap the repo for its sibling or `/pull/` for `/issues/` and the URL still renders, still looks plausible, and still points at somebody else's work or at nothing. `github.com/acme/widget-store/issues/1704` and `github.com/acme/widget-store-ui/pull/1704` differ in two segments and neither is obviously wrong on sight. Specifically: never derive `<repo>` from a folder basename, a product or multiproduct name, a Jira ticket's text, or a sibling repo (`-ui`, `-api`, `-service`, `-web` suffixes are _different repos_ with independently numbered PRs) — resolve it per Repo Identification; and never guess the type, because numbering is shared across issues and PRs in one repo, so `<n>` is at most one of the two, and a repo whose team never files issues has no `/issues/<n>` at all.
- **One probe checks all four at once, and its output is what you print:**
  ...
  The issues endpoint returns pull requests too, so `.pull_request` is the type discriminator and `.html_url` is already the canonical web path (`/pull/<n>` for a PR, `/issues/<n>` for an issue) — copy that field verbatim rather than assembling one. A `404` means at least one segment is wrong: try the sibling repo, say which candidates you tried, and **never print a link that did not resolve** — an unverified reference is reported as unverified, or not at all. Re-run the probe on any link that arrived from memory, a user paste, an earlier turn, a ticket, or a commit message; a link someone else typed is a hypothesis, not a source (Epistemic Honesty).
