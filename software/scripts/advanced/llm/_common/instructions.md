...
- One rigid worktree path, and exactly one thing that computes it — `git worktree-path <branch-or-pr> [<pr-number>]` prints `$HOME/.worktrees/<owner>/<repo>/<repo>__<slot>` and touches nothing, so a dispatcher can name a job's folder without creating anything. `<owner>` and `<repo>` come from the origin remote, never from the folder name. A direct number keeps legacy `pr-<number>` lookup; `git create-worktree <branch> <number>` uses `pr-<number>-<branch-slug>`, where the slug replaces every character outside `[A-Za-z0-9._-]` with `_`, collapses runs, and trims leading and trailing separators — `syle/oauth-migration/fix~it` becomes `pr-409-syle_oauth-migration_fix_it`. No variants, no per-command suffixes, no `mktemp -d`, no sibling-of-repo folders. Why the repo name is repeated in the leaf: every tool that shows only the basename — editor title bars, tmux windows, shell prompts, build project names — otherwise displays an ambiguous PR number. Why rigid: every command derives the same branch-aware path, so babysit / review / sync share one worktree per PR, it lives outside every repo so it never shows as untracked junk, and `ls "$HOME/.worktrees"` is the inventory. Clean up with `git clean-worktree` (prunes stale records, removes merged/gone branches, skips dirty ones); it skips detached-HEAD worktrees, so remove those with `git worktree remove`.
