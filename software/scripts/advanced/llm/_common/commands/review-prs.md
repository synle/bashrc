...
Start the **agent ledger** here: one row per resolved PR holding job state, dispatch time, finish time, and the **last rendered pulse snapshot** (Status color emoji, component lines, counters strip) that `/sy-list-prs pingpong` diffs to pick its change marker and `Δ` line — overwrite it after every pulse. Record dispatch and finish timestamps as they happen; the pulse's `started 17:12 · running 6m` clock is read from them and cannot be reconstructed later. `/sy-review-pr` is a **single pass**, so Agent cells carry no loop counter — `🔄 IN PROGRESS`, `✅ COMPLETED`, `⏭️ SKIPPED`, `❌ FAILED`.
...
- **Ping-pong layout is owned by `/sy-list-prs pingpong`.** This command owns the agent ledger (job state, dispatch / finish times, wave number, last rendered pulse snapshot) and passes it in; `/sy-list-prs` owns the three columns, the icons, the change marker, and the sort. `/sy-review-pr` is single-pass, so Agent cells here carry **no** loop counter — that `N/3` form belongs to `/sy-babysit-prs`.
