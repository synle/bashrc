...
- **Merges flow downhill only.** Never push an ancestor branch, never merge a branch into its parent, never merge or retarget an ancestor's PR to unblock a child, never edit an ancestor's migration. Cross-stack ordering is the caller's ≥10-pass loop, not a wait — never block on, message, or poll a sibling job.
