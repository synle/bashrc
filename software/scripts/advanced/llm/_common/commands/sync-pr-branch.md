...
`/sy-sync-pr-branch <PR-URL> workspace=$HOME/.worktrees/<owner>/<repo>/pr-<number> server-side=never report=structured`
...
5. **Workspace.** With `workspace=<path>`: `cd` there and use it as-is — do not create, reset, or clean it, and do not remove it on exit; the caller owns it (it has usually already run its own interrupted-run recovery).
   ...
   - `WT="$HOME/.worktrees/<owner>/<repo>/pr-<number>"` (`.../branch-<branch>`, `/` → `-`, when there is no PR); `mkdir -p "$(dirname "$WT")"`; `git worktree prune` first — a deleted folder leaves an orphan record that makes `git worktree add` fail with `already registered`.
   - Reuse a linked worktree already on `<headRefName>` (`git worktree list --porcelain`) if one exists. Else `git fetch origin <headRefName>` and create it — **`-B` only when `git rev-list --count origin/<headRefName>..<headRefName>` is `0` or the local branch doesn't exist**, since `-B` hard-resets and would destroy unpushed commits from an interrupted run; with local-only commits use `git worktree add "$WT" <headRefName>` (no reset) and reconcile via `git merge origin/<headRefName> --no-edit`. If the branch is checked out in the MAIN worktree, `git worktree add --detach "$WT" origin/<headRefName>` and push with `git push origin HEAD:<headRefName>` later. When cwd is not the target repo, `gh repo clone <owner/repo> "$WT"` and `gh pr checkout <number>` there.
...
6. **Merge the chain — one source at a time, in Step 3's order.** Regular merge commits: **never `--rebase`, never `--squash`, never an octopus `git merge A B C`** (a conflict there aborts everything and gives no per-source resolution point).
   ...
   - **Conflict resolution** (skip and report when `on-conflict=stop`):
     ...
     - Lockfiles / generated files (`package-lock.json`, `yarn.lock`, `Cargo.lock`, `.build/` artifacts): accept the incoming version and **regenerate** (`npm install`, `cargo build`, `make build`) rather than hand-editing.
...
- **All work happens in a worktree at `$HOME/.worktrees/<owner>/<repo>/pr-<number>` — never the user's primary checkout** (see One rigid worktree path). A caller-supplied `workspace=` is used as-is and never cleaned up; only a workspace this run created is removed on exit.
- **Resume an interrupted run, never discard it (Step 5).** Finish a left-over merge, commit work found at the canonical path, carry unpushed commits through the gate. `git worktree add -B` onto a branch with unpushed commits and `stash` / `reset` / `checkout --` to force a clean tree are both banned.
