[Sy] Sync a pull request branch with its base — the default branch plus, when the PR is stacked, every ancestor branch up the chain — address review comments, run local checks, fix failing builds, and wait until CI passes. All git work runs in a **dedicated worktree** (never your primary checkout), and the full loop runs **at least 3 passes, 30 minutes apart**. A standalone prose-only PR (docs / comments / whitespace) gets automerge enabled on the first pass; one with companion PRs still open waits for them, and anything with an executable line does not.

Argument: $ARGUMENTS (optional — a PR URL or PR number. If empty, use the current branch's PR. A caller dispatching a batch, e.g. `/sy-babysit-prs`, may also pass the full list of sibling PR URLs in that batch; Step 2a's companion check treats them as companions.)
...
5. **Sync the PR branch with everything it sits on — the default branch, plus every ancestor branch when the PR is stacked — resolve conflicts, and push** (NEVER rebase, must not rewrite history). **All git work for THIS PR happens inside a dedicated worktree (or a scoped clone)** so the user's primary checkout and current branch are never disturbed.
   ...
   **⚠️ A STACKED PR HAS MORE THAN ONE SOURCE.** Merging only the default branch into a child of a stack leaves it missing everything the parent pushed since it was cut; merging only the parent leaves the whole stack behind the default branch. Step 5c resolves the chain, Step 5d orders it, Step 5e merges every source in that order.
   ...
   - On final exit — **after the last pass (Step 12), not between passes** — clean up **only the workspace this run created**:
     ...
     - Anything left behind is still reachable via `ls "$HOME/.worktrees"` and reapable by the `git clean-worktree` alias (skips dirty worktrees; skips detached HEADs, so remove those explicitly).
       ...
       c. **Resolve the ancestor chain BEFORE merging anything — a stacked PR has more than one branch to sync from.** Run this every pass, not once: bases move, parents push, parents merge, and GitHub retargets a child the moment its parent lands.
   - Resolve the default branch once, never assume `main`:
   ...
   - Re-read this PR's base fresh — `gh pr view <number> --repo <owner/repo> --json baseRefName --jq .baseRefName` — do not reuse a value cached from an earlier pass.
   - **Walk upward.** With `B` = this PR's base, loop: if `B == $DEF`, stop. Otherwise ask whether `B` is itself the head of an open PR — `gh pr list --repo <owner/repo> --head "$B" --state open --json number,url,title,headRefName,baseRefName`. A hit means this PR is **stacked**: record `B` as an ancestor, set `B` to that PR's `baseRefName`, and keep walking. Stop on: `B == $DEF`, a `B` with no open PR, a `B` already seen (cycle), or **10 hops** (hard cap — report and stop, never loop forever).
   - The walk yields `ancestors[]` — immediate parent first, root-most last — and a **floor**, the branch the walk stopped on:
     - Stopped at `$DEF` → floor is `$DEF`. The normal case, stacked or not.
     - Stopped at a non-default branch with **no open PR** (a long-lived integration / release line, or a parent whose PR merged or was closed) → **that branch is the floor and `$DEF` is NOT merged.** Merging the default branch into a release-line PR imports commits that line never asked for. Record it as the floor, say so, and move on.
     - Stopped because the branch no longer exists on the remote (`git ls-remote --heads origin "$B"` prints nothing) → the parent landed and its branch was deleted; GitHub has already retargeted this PR. Re-read `baseRefName` once more and restart the walk from the new base.
   - **A parent PR that is `CLOSED` without merging is a red flag, not a blocker.** Report `"base <branch> belongs to PR <link>, closed without merging — this PR is stacked on abandoned work and should be retargeted"`, skip that ancestor as a merge source, and carry on with the rest of the chain. Surface it in the Step 13 report; do not wedge the loop on it.
   - Report the shape once per pass: `"not stacked — base is <DEF>"`, or `"stacked <n> deep: this PR ← <parent link> ← <grandparent link> ← <floor>"`.
     d. **Build the merge order: floor first, then every ancestor from root-most down to the immediate parent.** For a non-stacked PR that list is one entry (`$DEF`) and this is the ordinary base merge. For a stacked PR it is the whole chain, bottom-up — the floor's commits arrive first so each ancestor merge lands on top of a base that already has them, which is what keeps the conflict set small and resolved once instead of once per level.
   - **Merges flow downhill only.** Ancestor → this branch, never the reverse. Never merge this PR's branch into its parent, never `git push` an ancestor branch, and never `gh pr merge` an ancestor to "unblock" this one — those branches belong to other PRs (often other babysit jobs), and this job owns exactly one branch.
   - Ordering across a stack is handled by the loop, not by waiting: if a parent pushes after this pass read it, the next pass (Step 12, 30 min later) picks it up. Never sleep on a sibling job or ask it for status.
     e. **Merge each source in that order, one at a time** (regular merge commits — do NOT use `--rebase`, do NOT use `--squash`), resolving conflicts per Step 5f **after each merge, before starting the next**:
   - `git fetch origin "<source>"` then `git merge "origin/<source>" --no-edit`.
   - `Already up to date` is a normal, common result on the upper entries — the immediate parent usually already carries its own ancestors' commits. Log it and move to the next source; never skip a source just because you expect it to be empty.
   - Never collapse the chain into one `git merge A B C` octopus merge — a conflict in an octopus merge aborts the whole thing and gives no per-source resolution point.
   - After the last source, `git status` must show a clean tree, and `git log --oneline -1` a merge commit (or the pre-merge HEAD when everything was already up to date).
   - Report each source's result: `"merged <source> (<n> commits)"` / `"<source>: already up to date"`.
     f. **If merge conflicts occur, resolve them — this is the primary goal of this step:**
   ...
   - Verify the merge is complete: `git status` should show a clean working tree with no "unmerged paths".
     g. **Check migration head divergence** (if the project uses a migration tool). After the Step 5e merges, the PR's migration DAG must have exactly one head that descends from the floor branch's current head. Never push a branch that would create a second migration head on main/master. Detect a migration tool (first match wins; skip the whole sub-step if none found):
     ...
     Verify: re-run the head/divergence check — must now report a single head descending from base. Commit the re-parented migration. If re-parenting needs judgment you don't have (conflicting schema semantics, destructive ordering, ambiguous intent): stop and ask the user — same escape hatch as the text-conflict resolver in Step 5f. Never guess a migration re-order.

     **Cross-PR angle (stacks):** run this check once after the **whole** Step 5e chain has merged, not per source, and treat the ancestors' migrations as the parents: each ancestor PR's migration must be an ancestor of this PR's, single-head. If an ancestor PR and this PR add sibling migrations, that's the review-time coordination case `/sy-review-pr` already warns about; re-parent **this** PR's migration onto the ancestor's and surface it in the final report — never re-parent or edit a migration that belongs to an ancestor PR (Step 5d, merges flow downhill only).
...
- The loop is: Step 2a prose-only automerge gate (first pass; the deferred-companion branch re-checks every pass) → Step 3 skip-if-running → Step 4 early-exit (or, when automerge is already on, wait-for-merge) → Step 5 worktree setup + ancestor-chain resolve + merge floor then every ancestor + migration-head check + push → Step 6 human comments → Step 7 trivial bot comments → Step 8 add/update tests + best-effort coverage check → Step 9 pre-emptive CI fix → Step 10 local checks → Step 11 CI monitor → (on failure) back to Step 3 → Step 12 loop gate (≥3 passes, 30 min apart). On `state == "MERGED"`: report the merge SHA, invoke `/sy-release` (auto-release after merge), and stop.
...
- **A stacked PR syncs from its whole ancestor chain, floor first, immediate parent last (Step 5c–5e).** Right after the default-branch merge, a PR whose base is itself an open PR's head merges every ancestor branch too — root-most down to the immediate parent, one `git merge` per source with conflicts resolved between them. Merging only the default branch leaves a child missing everything its parent pushed; merging only the parent leaves the stack behind the default branch. Re-resolve the chain **every pass** — parents push, merge, and get retargeted mid-run. Depth cap 10 hops, cycles stop the walk.
- **The floor is whatever the walk stops on, not always the default branch.** Stop at the default branch → merge it. Stop at a long-lived non-default branch with no open PR (a release / integration line) → that branch is the floor and the default branch is **not** merged; importing main into a release-line PR drags in commits that line never asked for. Stop at a deleted branch → the parent landed and GitHub retargeted this PR: re-read `baseRefName` and restart the walk. An ancestor whose PR was **closed without merging** is reported as orphaned base and skipped as a source — never a reason to wedge the loop.
- **Merges flow downhill only — this job owns exactly one branch.** Ancestor → this PR, never the reverse: never merge this branch into its parent, never push an ancestor branch, never `gh pr merge` an ancestor to unblock this one, never edit an ancestor's migration. Those branches belong to other PRs and usually other babysit jobs. Cross-stack ordering is handled by the ≥3-pass loop — a parent that pushes after this pass read it is picked up 30 minutes later. Never wait on, message, or poll a sibling job.
...
- **Migration head stays single — after the Step 5e chain merges, the PR's migration DAG must have exactly one head that descends from the floor branch.** Re-parent (don't add a merge revision) to keep it linear; never create a second head on main/master, and never re-parent a migration belonging to an ancestor PR. `/sy-review-pr` warns about cross-PR migration conflicts; `/sy-babysit-pr` fixes them (Step 5g).
