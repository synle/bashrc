[Sy] Sync a pull request branch with its base (main/master), address review comments, run local checks, fix failing builds, and wait until CI passes. All git work runs in a **dedicated worktree** (never your primary checkout), and the full loop runs **at least 3 passes, 30 minutes apart**. A standalone prose-only PR (docs / comments / whitespace) gets automerge enabled on the first pass; one with companion PRs still open waits for them, and anything with an executable line does not.
