...
4. **Early-exit check.** Fetch state:
   ...
   - If `state == "MERGED"`: skip ahead to **Step 12 (post-merge release trigger)**. Nothing else to babysit.
...
5. **Merge base into the PR branch AND RESOLVE CONFLICTS** (NEVER rebase, must not rewrite history).
   **The whole point of this step is to surface and fix merge conflicts before CI does.** If `git merge` reports conflicts, you MUST resolve them — do not skip, do not abort, do not leave the branch in a conflicted state.
   ...
   d. **If merge conflicts occur, resolve them — this is the primary goal of this step:**
...
7. **Address ONLY trivial / minor bot comments:**
   ...
   - **Same reply + resolve pattern as Step 6** for every trivial bot fix applied: `Fixed — <one-liner>` reply, then resolve the thread. Skipped bot comments — leave unaddressed (no reply, no resolve).
...
12. **Post-merge release trigger** (per global rule 47):
...
- The loop is: Step 3 skip-if-running → Step 4 early-exit (or wait-for-automerge-merge) → Step 5 merge base → Step 6 human comments → Step 7 trivial bot comments → Step 8 add/update tests → Step 9 pre-emptive CI fix → Step 10 local checks → Step 11 CI monitor → (on failure) back to Step 3. On `state == "MERGED"`: jump directly to Step 12 post-merge release.
...
- **Every behavior-changing fix from Step 6 or 7 needs test coverage in Step 8.** Doc/typo/format-only changes are exempt.
...
- **After every applied fix: reply `Fixed — <one-liner>` and resolve the thread** (mechanics in Step 6). Reply-without-fix → post reply, leave thread open.
