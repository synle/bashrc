...
Start the **agent ledger** here: one row per resolved PR holding job state, loop number, last pass start / end, next pass ETA, and the **last rendered pulse snapshot** (Status verdict token, reason clause, counters strip). The snapshot is what `/sy-list-prs pingpong` diffs to pick the `🔴*` / `▫️` / `🆕` change marker and write the `Δ` line — overwrite it with what was just printed after every pulse, so each pulse compares against the one immediately before it. The ledger is this command's bookkeeping; `/sy-list-prs` only renders it.
