...
- **Prose-only PRs in the set will self-merge.** `/sy-babysit-pr` Step 2a enables `--auto` on any docs / comment / whitespace-only PR it picks up, so a fan-out over a docs sweep merges those PRs as their CI goes green. Call that out in the Step 2 announce so the user sees it coming; the dispatcher never enables or disables automerge itself.
