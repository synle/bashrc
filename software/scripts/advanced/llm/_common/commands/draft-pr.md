...
- **Sibling PRs collide on files, not just on bases (Step 4b).** The no-stack pre-flight only catches a shared *ancestor*; two perfectly independent PRs editing the same file still conflict. A WIP is the cheapest possible moment to fix that, since nothing has been reviewed yet and reshaping costs nothing — check the file lists against your other open PRs and design the overlap out per the "Make slices disjoint with new files, not with discipline" rule. Advisory, never a blocker.
