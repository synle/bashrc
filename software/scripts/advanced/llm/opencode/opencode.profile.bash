...
Related files NOT inside the dir:
  ~/.local/share/opencode/opencode.db - SQLite store for sessions/history.
  ~/.auth-opencode.json               - real credential store (provider logins).
                                        ~/.local/share/opencode/auth.json is a
                                        symlink onto it, relinked by
                                        opencode/setup.js, so a reinstall can't
                                        wipe your logins."
    return 0
  fi
...
clipboard (via the universal copy helper). Paste it back into opencode with
Cmd/Ctrl+V."
    return 0
  fi
