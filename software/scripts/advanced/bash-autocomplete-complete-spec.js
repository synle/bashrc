...
async function doWork() {
  ...
  for (const entry of SPEC_COMMANDS) {
    ...
    // Resolve `>__name__|...` static macros at build time so the runtime bash
    // parser only sees fully-expanded command lines (no `>` lines, no static
    // flag tokens like `__git_diff_flags__`).
