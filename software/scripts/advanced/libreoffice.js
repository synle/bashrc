...
function _getLibreOfficeUserDir() {
  ...
  try {
    ...
    if (versions.length > 0) {
      ...
      if (pathExists(userDir)) return userDir;
    }
  } catch (err) {}
...
async function doWork() {
  ...
  const sofficeRcPaths = [
    ...
    `${BASE_PROGRAM_FILES_WINDOW}/LibreOffice/program/sofficerc`, // Windows (WSL)
  ];
  for (const sofficeRcPath of sofficeRcPaths) {
    if (pathExists(sofficeRcPath)) {
      log(">>> Disable splash screen (sofficerc):", sofficeRcPath);
      await backupConfigFile(sofficeRcPath);
      let rcContent = await readText`${sofficeRcPath}`;
      rcContent = rcContent.replace(/Logo=[0-9]/, "").trim();
      rcContent += `\nLogo=0`;
      rcContent = rcContent.replace(/[\n][\n]+/, "\n");
      await writeText(sofficeRcPath, rcContent);
      break;
    }
  }
  ...
  await backupConfigFile(registryPath);
  ...
  let regContent = await readText`${registryPath}`;
  const closingTag = "</oor:items>";

  /** @param {string} oorPath - OOR registry path @param {string} propName - Property name @param {string} value - Value to set */
