...
/**
 * Returns Chromium Preferences that are SAFE for all Chromium-based browsers (Brave, Chrome, Edge).
 * No setting here should weaken security or break common workflows (sync, Cast, account sign-in).
 * Items that WOULD affect those — safebrowsing/signin/media_router — live in _getBraveConfigs().
 * @returns {object} Common Chromium Preferences to merge for any Chromium-based browser.
 */
function _getChromiumConfigs() {
  return {
    ...
    // =========================================================================
    // enable_hyperlink_auditing — Privacy (mirrors --no-pings launch flag)
    // =========================================================================
    enable_hyperlink_auditing: false, // Blocks <a ping=""> beacons; no site relies on these

    // =========================================================================
    // translate — Languages (mirrors --disable-features=Translate launch flag)
    // =========================================================================
    translate: {
      enabled: false, // Languages > "Offer to translate pages in other languages" = OFF (right-click → Translate still works)
    },

    // =========================================================================
    // optimization_guide — Privacy telemetry (mirrors --disable-features=OptimizationHints)
    // =========================================================================
    optimization_guide: {
      fetching_hints_enabled: false, // Stop fetching Google's page-optimization hints (telemetry)
    },
...
function _getBraveConfigs() {
  return {
    // =========================================================================
    // Shared Chromium prefs that are Brave-safe but NOT safe defaults for Chrome/Edge.
    // Brave has its own mitigations (Shields, no Google-account dependency, etc.).
    // =========================================================================
    safebrowsing: {
      enabled: false, // Privacy > "Safe Browsing" = OFF — Brave Shields replaces this; DON'T do this on Chrome/Edge
    },
    signin: {
      allowed: true, // Settings > "Allow browser sign-in" = ON (enables sync via Brave Sync / Google account)
    },
    media_router: {
      enable_media_router: false, // Settings > "Google Cast" = OFF — Chrome/Edge users may actually want Cast
    },
 ...
 * Safety note: Chrome users typically depend on Google-account sync; don't touch signin here.
...
function _getChromeConfigs() {
  return {
    ...
    // =========================================================================
    ntp: {
      shortcuts_visible: false, // New Tab Page > "Show shortcuts" = OFF (Chrome's real key — not Brave's misspelled `shortcust_visible`)
    },
  };
}
 ...
 * Safety note: Edge users typically depend on Microsoft-account sync; don't touch signin here.
 * Candidates to research before adding: Copilot sidebar button, Collections pane, Bing visual search,
 * `edge.shopping_assistant.*`, Discover/Feed on new tab page.
...
function _getBrowserConfigs(browserName, existingPrefs = {}) {
  ...
  const isBrave = name.includes("brave");
  const isChrome = !isBrave && name.includes("chrome");
  const isEdge = name.includes("edge");

  const chromium = _getChromiumConfigs();
  let browserSpecific = {};
  if (isBrave) browserSpecific = _getBraveConfigs();
  else if (isChrome) browserSpecific = _getChromeConfigs();
  else if (isEdge) browserSpecific = _getEdgeConfigs();

  // Merge: existing → chromium common → browser-specific
  const merged = _deepMerge(_deepMerge(existingPrefs, chromium), browserSpecific);

  // Brave-only: wipe + replace accelerator keymap (the `brave.accelerators` namespace only
  // exists in Brave — Chrome/Edge would store it as dead JSON and ignore it).
  if (isBrave) {
...
async function doWork() {
  const browserProfilePaths = [_getBraveProfilePath(), _getChromeProfilePath(), _getEdgeProfilePath()];
