...
Goal: one consistent format engine per file type across editors. **oxc (oxfmt) owns
JS/TS/JSX/TSX/JSON/HTML/CSS/SCSS/LESS** — the same set the `curl()`/oxfmt CLI path uses in
`profile-advanced.sh`. **Prettier keeps JSONC/MD/YAML/GraphQL/Vue** (oxfmt still mangles
MD/YAML; JSONC/XML intentionally stay on Prettier). Native LSP formatters cover
Python/Rust/Go/Java. Save and the unified format chord (`ctrl+shift+OS_KEY+f` —
`Ctrl+Shift+Cmd+F` mac / `Ctrl+Shift+Alt+F` linux) route through the **same engine** per
file type in every editor. Sublime is NOT wired for oxc (no Package Control package
exists) — it stays on Prettier + eslint.

| Editor       | oxc types (JS/TS/JSON/HTML/CSS/SCSS/LESS)   | Prettier types (JSONC/MD/YAML/GraphQL/Vue) | Native LSP types (Python/Rust/Go/Java) | Save trigger                                            | Chord trigger                                                                             |
| ------------ | ------------------------------------------- | ------------------------------------------ | -------------------------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| VS Code      | `oxc.oxc-vscode` extension (oxlint + oxfmt) | `esbenp.prettier-vscode`                   | Per-language LSP extension             | `editor.formatOnSave: true`                             | `editor.action.formatDocument` → honors `editor.defaultFormatter` / `[lang]`              |
| Zed          | `oxc` extension — `oxfmt` language server   | Built-in prettier (`prettier.allowed`)     | Built-in LSP formatter                 | `format_on_save: "on"` + `languages.<X>.formatter`      | `editor::Format` → honors `formatter` setting                                             |
| Sublime Text | n/a — not wired (no oxc package)            | `JsPrettier` package (runs `prettier` CLI) | `LSP` package + per-language `LSP-*`   | JsPrettier `auto_format_on_save` + `lsp_format_on_save` | Context-bound chord — `js_prettier` for prettier syntaxes, `lsp_format_document` fallback |

oxc binaries: `oxlint` + `oxfmt` are installed globally (`software/scripts/advanced/oxlint.sh`,
`oxfmt.sh` → `npm_install_global`). The VS Code extension has no PATH fallback, so
...
- Zed: `software/scripts/advanced/zed-config.jsonc` (`auto_install_extensions.oxc`; `languages.<X>.formatter` → `{ language_server: { name: "oxfmt" } }` for the oxc types with `source.fixAll.oxc` chained for JS/TS/TSX; `"prettier"` for JSONC/MD/YAML; `prettier.allowed: true`; `format_on_save: "on"` global).
...
| ------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `oxc.oxc-vscode`                | oxlint (lint + fix-on-save) + oxfmt (format) — owns JS/TS/JSX/TSX/JSON/HTML/CSS/SCSS/LESS | `~/Library/Application Support/Code/User/settings.json` → `[lang]` `editor.defaultFormatter`, `codeActionsOnSave["source.fixAll.oxc"]`, `oxc.path.oxlint`/`oxc.path.oxfmt` | `software/scripts/advanced/vs-code-config.jsonc`, `vs-code.js` |
| `esbenp.prettier-vscode`        | Prettier integration — owns format for JSONC/MD/YAML/GraphQL/Vue                          | `~/Library/Application Support/Code/User/settings.json` → `editor.defaultFormatter`, `[lang]` overrides                                                                    | `software/scripts/advanced/vs-code-config.jsonc`               |
