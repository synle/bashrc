...
async function doWork() {
  ...
  if (!fs.existsSync(targetPath)) {
    console.log(
      "  >> Skipped Windows Terminal Config - Settings Path Not Found: ",
      targetPath
    );
    process.exit();
  }
...
function _getPath() {
  try {
    if (is_os_window) {
      const localPackgesPath = path.join(
        getWindowUserBaseDir(),
      ...
      return path.join(
        localPackgesPath,
        fs
          .readdirSync(localPackgesPath)
          .filter((dir) => dir.indexOf("Microsoft.WindowsTerminal") >= 0)[0]
      );
    }
    return null;
  } catch (e) {
