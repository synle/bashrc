...
#            Conflicts (dest file replaced) are logged as OVER. Progress, ETA,
#            cross-device safe.
...
function cpsync() {
  ...
  Per-file log: SKIP (unchanged), COPY (new in dest), OVER (conflict, dest file replaced), FAIL
  Options:
...
let copied = 0, overwritten = 0, skipped = 0, failed = 0, copiedBytes = 0, skippedBytes = 0, totalBytes = 0;
...
for (let i = 0; i < files.length; i++) {
  ...
  if (skip) {
    ...
    try {
      ...
      copied++;
      copiedBytes += srcSize;
      // dest already had this file with different content: report it as a conflict overwrite
      if (destExists) overwritten++;
      const label = destExists ? ' OVER ' : ' COPY ';
      const outcome = destExists ? ' -> Overwritten (conflict)' : ' -> Copied';
      console.log(prefix + label + file + ' -> ' + destFile + ' (' + sizeStr + wcStr + ')' + outcome);
    } catch (e) {
      failed++;
...
if (overwritten > 0) console.log('  Overwrote: ' + overwritten + ' existing dest files (conflicts)');
