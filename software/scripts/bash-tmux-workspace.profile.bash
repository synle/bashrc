...
# echo the controlling terminal's REAL device path (/dev/ttys004, /dev/pts/3)
## deliberately NOT "/dev/tty". tmux takes the client's identity from
## ttyname(stdin) (client.c), and an fd opened on the /dev/tty alias reports back
## as "/dev/tty" verbatim on macOS - which the server rejects outright with
## "open terminal failed: can't use /dev/tty" (server-client.c,
...
## stdin is re-pointed at the real terminal device whenever it is not already a
## terminal: the documented `workspace_create - << 'JSON_EOF'` form leaves the
## heredoc on fd 0, and tmux then dies with "open terminal failed: not a
## terminal". Without a controlling terminal at all there is nothing to attach
## to, so say so instead.
## the redirect is `<>` (read-WRITE), never `<`: the server draws the whole UI
## back down the descriptor it got from the client's fd 0, so a read-only open
