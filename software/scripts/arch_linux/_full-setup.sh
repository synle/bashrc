...
_BACKGROUND_INSTALL_LOG="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_pacman_$$.log"
_BACKGROUND_INSTALL_SCRIPT="$BASHRC_TEMP_ROOT_DIR/bashrc_bg_pacman_$$.sh"
...
installPacmanPackage shellcheck # used by `make ci_test_shellcheck`
...
# --- Multimedia ---
# ffmpeg is `required` per ci-binaries.json — install foreground so binary verification
# in CI doesn't race the 5-minute _waitForBackgroundPackages cap. Yes it pulls heavy
# transitive deps, but reliability > install-time parallelism.
...
if has_a_gui; then
  ...
  if has_a_gui x11; then installPacmanPackageInBackground xclip; fi
  if has_a_gui wayland; then installPacmanPackageInBackground wl-clipboard; fi
...
# --- Power Management ---
################################################################################
_configureDisableSystemdPowerAndTurboBoost
