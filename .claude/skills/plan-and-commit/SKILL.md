...
description: Write a plan, execute it, commit the result, and leave behind plan-YYYY-MM-DD-<slug>.md plus plan-YYYY-MM-DD-<slug>.diff in ~/sy/ai_llm/plans/<repo>/. Use for any multi-file or multi-step change where the reasoning is worth recording alongside the code.
---
...
Then emit the diff — `$PLANS` is `~/sy/ai_llm/plans/<repo>`:
...
- **One task, one slug.** Don't overwrite an existing `~/sy/ai_llm/plans/<repo>/plan-YYYY-MM-DD-<slug>.*` pair from earlier work — pick a distinct slug or ask.
