...
if [ -n "$W_PATH" ] && [ -d "$W_PATH" ]; then
    ...
    curl -fsSL "$REPO_BUILD/sublime-text-mousemap" -o "$W_PATH/Default.sublime-mousemap"
    ...
    curl -fsSL "$REPO_BUILD/sublime-text-mousemap" -o "$L_PATH/Default.sublime-mousemap"
    ...
    curl -fsSL "$REPO_BUILD/sublime-text-mousemap-macosx" -o "$M_PATH/Default.sublime-mousemap"
...
if (Test-Path $W_Path) {
    ...
    Invoke-WebRequest -Uri "$RepoBuild/sublime-text-mousemap" -OutFile "$W_Path/Default.sublime-mousemap"
