# NOTE: STOP - do not edit by hand - this file is auto-generated [2026-04-14]
# 
...
# Generated: 2026-04-14T15:46:19.617Z
# END Profile Generated Timestamp
#
################################################################################
# SOURCE_BEGIN software/scripts/bash-history-profile.bash
# software/scripts/bash-history-profile.bash | 69d927130541f9e971986185f6f597d2 | 5.5 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
...
# software/scripts/bash-path-candidate-profile.bash | 4d2b818bb6e5868bcf7d94b2b6f4b20d | 3.5 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
################################################################################
# ---- PATH Setup ----
# The system already sets PATH before this script runs. We prepend our
# candidates, then dedupe (first-seen wins, so user tools take precedence
# over system packages) and prune non-existent directories.
################################################################################
...
# software/scripts/bash-keys-profile.bash | 48ea5e38a751e3a40ffb6d5a1556ef8c | 4.0 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
# software/scripts/bash-file-utils.bash | 8ed78e464d1257038c4811d459d4e230 | 27.9 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
# _cp_zip_to_dest — Zip and copy to dest. Accepts a folder (zips everything) or a
#            temp file list of absolute paths (zips only those). Handles the full
#            flow: zip creation, size check, cpsync to dest, cleanup of tmp files.
...
# software/scripts/bash-fzf-profile.bash | d9b5cc35c21c01730649c3436573c43b | 16.1 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
# software/scripts/advanced/editor-launchers-common.bash | 900bf2c78b7d998ba525a68d825171af | 2.2 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
# perf: avoid bash while-read loops for processing command output (e.g.
#   cmd | while read -r line; do ...; done). Each iteration spawns in a
#   subshell and is extremely slow with thousands of lines. Use native
#   tools instead: sed for substitution, tr for delimiters, grep for
#   filtering, awk for field extraction. These process the entire stream
#   in a single pass without per-line bash overhead.
...
# tab-completion handler for cat — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
ssh_hosts=$(command grep -i '^Host ' ~/.ssh/config | awk '{for(i=2;i<=NF;i++) print $i}' | command grep -v '[*?]' | __to_opts_sorted)
fi
if [ -d ~/.ssh/config.d ]; then
local extra_hosts=$(command grep -i '^Host ' ~/.ssh/config.d/* 2> /dev/null | awk '{for(i=2;i<=NF;i++) print $i}' | command grep -v '[*?]' | __to_opts_sorted)
...
local _max_depth="${BASHRC_AUTOCOMPLETE_MAX_DEPTH:-3}"
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for bat — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for batcat — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for less — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for vim — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for code — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for subl — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for zed — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for cd — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for eza — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for ls — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for tree — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for zoxide — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for fd — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for jq — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for rg — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for delta — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for g — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for gh — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for git — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for brew — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for cargo — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for fnm — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for n — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for npm — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for npx — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for uv — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for y — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for yarn — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for gmake — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for gradle — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for gradlew — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for make — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for kubectl — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for curl — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for rsync — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for s — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for ssh — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for adb — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for sqlite3 — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for tldr — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# same as __to_opts but deduplicates and sorts first
function __to_opts_sorted() { sort -u | tr '\n' ' '; }
# tab-completion handler for tmux — resolves spec data, expands
# dynamic tokens (git branches, files, npm scripts, etc.), and populates COMPREPLY
...
# software/scripts/bash-command-wrappers-profile.bash | 19f25048c9ea4f9f5407b0dd9355d4a4 | 4.8 KB | 2026-04-14
# SOURCE_BEGIN software/bootstrap/common-functions.bash
# software/bootstrap/common-functions.bash | 0af03bee8a417a98aca0034a3e0a5f31 | 4.5 KB | 2026-04-14
# Shared shell functions for run.sh and SH scripts (via SOURCE markers).
# Source of truth — inlined into run.sh via BEGIN/END, included in .sh scripts at runtime.
...
# SOURCE_END software/bootstrap/common-functions.bash
# ---- Command Wrappers ----
# Lazy-activation wrappers for python, node, npm, yarn, and related tools.
# These shadow the real binaries so the first invocation triggers setup (e.g.
# activating a venv or fnm), then delegates to the real command.
#
# Sourced AFTER spec-based autocomplete so that `complete -F ... <cmd>` bindings
# are already registered before these function wrappers are defined.
