...
| `super+l`, `ctrl+l`                                  | Clear input               |       ✅       | ❌<sup>n</sup> |       ⚠️       | ✅<sup>i</sup>  |
| `escape` (opencode only)                             | Clear input               |       ❌       | ✅<sup>n</sup> |       ❌       |       ❌        |
| `ctrl+l`, `super+l`, `ctrl+o c` (opencode only)      | Compact / reset chat      |       ❌       | ✅<sup>o</sup> |       ❌       |       ❌        |
...
<sup>n</sup> OpenCode's `input_clear` is bound to `escape` **only** — the `super+l` / `ctrl+l` l-chords are moved to `session_compact` (see <sup>o</sup>). `escape` mirrors Claude/Copilot muscle memory (where a **double-tap** of escape wipes the input); OpenCode has no double-tap / key-sequence mechanism (the only sequence is the leader key), so exact double-escape parity is impossible — escape is bound as a **single** press instead. OpenCode context-scopes chords, so this coexists with the upstream default `session_interrupt` = `escape` (kept intact): escape clears the input when idle and still interrupts a streaming response. TRADEOFF: single escape is more aggressive than Claude's double-tap, so an accidental escape clears the field; `input_undo` (`super+z` / `ctrl+-`) recovers it. (Claude/Copilot/Gemini keep `super+l`/`ctrl+l` = clear-input; only OpenCode diverges, because its l-chords are repurposed for compaction.)
<sup>o</sup> On Claude/Copilot the l-chords (`ctrl+l` / `cmd+l`) visually clear the chat transcript. OpenCode has **no** clear-transcript command (only "Clear input field" and "Clear editor context"), so `ctrl+l` and `super+l` route to `session_compact` — it summarizes + condenses the conversation so far, freeing the context window while keeping continuity. The nearest "reset the chat so far" that doesn't drop context. `<leader>c` (opencode's upstream default, typed `ctrl+o c` after the leader move) is kept explicitly so the binding this file writes doesn't drop it.
