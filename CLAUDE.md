...
- **New `.sh`/`.bash` scripts in `software/scripts/` must start with `#!/usr/bin/env bash` and `# SOURCE software/bootstrap/common-functions.bash`.** This ensures shared functions (`curl_bash_install`, `is_force_refresh_stale`, etc.) are available at runtime. Only omit the SOURCE line if the script genuinely uses none of those functions.
...
- **Prefer `curl -fsSL <url> | bash` installers over package managers (npm, pip, etc.) for CLI tools.** Official installer scripts are more reliable and up-to-date than package manager distributions. Only fall back to `npm install --global` or similar when no official installer exists.
