...
- **Use `safe_chown` and `safe_chmod` instead of `chown`, `chmod`.** Both skip paths that do not exist. `safe_chown [-R] <path...>` runs `sudo chown $USER`. `safe_chmod [-R] <mode> <path...>` runs `chmod`. One path per line for consistency.
