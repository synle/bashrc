...
- **Presets** (`software/metadata/presets.json`): Named bundles for `--preset=<name>` — each entry has a `files[]` list and optional `description`. Current presets: `lightweight` (minimal config: git/vim/inputrc), `editors` (vim/sublime/zed/vs-code/launchers), `browsers` (browser-config + launchers), `terminal` (inputrc/fzf/autocomplete). Multiple presets compose: `--preset=editors,browsers` unions their file lists. `printRunInfo` prints each chosen preset's description + file list before the run. To add a preset: edit `presets.json`, run `make format`, run `bash run.sh --preset=<new-name>`.
